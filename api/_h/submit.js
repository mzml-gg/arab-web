...
async function publish({ title, description, language, code, video_url, u, isAdmin }) {
...
module.exports = async (req, res) => {
  ...
  try {
    const { title, description, language, code, video_url } = await readBody(req);
    ...
    if (isVerified || settings.auto_approve) {
      const filename = await publish({ title, description, language, code, video_url: isVerified ? video_url : null, u, isAdmin });
    ...
    const entry = {
      ...
      code,
      video_url: isVerified ? video_url : null,
      author: u.username,
      author_avatar: u.avatar_url || null,
